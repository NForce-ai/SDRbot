...
class TokenTracker:
    ...
    def add(
        ...
        *,
        ...
        Args:
            input_tokens: Total input tokens to add to session (may be accumulated across subagents)
            output_tokens: Total output tokens to add to session (may be accumulated across subagents)
            context_input: Input tokens from last LLM call (for current context window size).
                          If None, uses input_tokens.
            context_output: Output tokens from last LLM call (for current context window size).
                           If None, uses output_tokens.
        """
        # Current context = last call's context window (for /context display)
        ctx_in = context_input if context_input is not None else input_tokens
        ctx_out = context_output if context_output is not None else output_tokens
        self.current_context = ctx_in + ctx_out
        self.last_output = ctx_out
        # Session total accumulates all tokens (including subagent calls)
