...
def lusha_enrich_company(domain: str) -> str:
    ...
    try:
        ...
        info = [
            ...
            f"Social: {c.get('social', {}).get('linkedin')}",
...
def lusha_prospect(filters_json: str) -> str:
    ...
    try:
        ...
        # Assuming /prospecting/search endpoint (hypothetical standard for Lusha, checking docs...)
        # Note: Lusha Enterprise API usually exposes 'prospecting' or 'search' endpoints.
        # If standard plan doesn't support search via API, this might fail, but let's assume standard enterprise access.
        
        # Mapping to typical search payload
        payload = {"filters": filters, "limit": 10}
        
        # Using a generic search endpoint name, might vary by plan level
